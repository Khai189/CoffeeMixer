...
const difficultyColors = {
    easy: "bg-green-100/90 text-green-800 dark:bg-green-900/90 dark:text-green-300",
    medium: "bg-amber-100/90 text-amber-800 dark:bg-amber-900/90 dark:text-amber-300",
    hard: "bg-red-100/90 text-red-800 dark:bg-red-900/90 dark:text-red-300",
};
            ...
            <div className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <div className="min-w-0">
                        <Link
                            to={`/recipe/${id}`}
                            className="font-semibold text-gray-900 dark:text-white hover:text-amber-600 dark:hover:text-amber-400 transition-colors truncate block"
                        >
                            {name}
                        </Link>
                        <p className="text-xs text-gray-500 dark:text-gray-400 truncate">
                            {authorId ? (
                                <Link to={`/user/${authorId}`} className="hover:text-amber-600 dark:hover:text-amber-400">
                                    {author}
                                </Link>
                            ) : (
                                author
                ...
                {ingredients.length > 0 && (
                    <div className="flex flex-wrap gap-1.5" aria-label="Ingredients">
                        {ingredients.slice(0, 4).map((ingredient) => (
                            <span key={ingredient} className="text-xs px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 border border-amber-200/60 dark:border-amber-800/40">{ingredient}</span>
                        ))}
                        {ingredients.length > 4 && (
                            <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500">+{ingredients.length - 4}</span>
                        )}
                    </div>
                )}
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
                    <span className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                        <span aria-hidden="true">{brewMethodEmoji[brewMethod] ?? "☕"}</span>
                        {brewMethod}
                    </span>
